...
OR build a straight apk:

eas build --platform android --profile preview2 --local
adb install /Users/vincent/develop/.../build-xxxxxxxxxxxxx.apk
...
Or in file format (package.json) :
"@vgerris/expo-alarm": "file:../expo-alarm",


## Cleaning the source dir
rm -rf node_modules yarn.lock ios android .expo .tscache
yarn install
npx expo prebuild --clean
cd ios && pod install # optionally build the iOS pods
yarn install

### Run Android emulator - DNS option

emulator -avd Medium_Phone_API_35 -dns-server 8.8.8.8
Note : Android when referring to localhost refers to itself, iOS to the computer localhost

#### iOS - setup building
run:
xed ios # opens Xcode
Go to : Product - Scheme - Edit Scheme , then for Run on the left, under Info select Build Configuratio - Release
Click on the app on the top left, Signing and capabilites tab - select a Team - connect the phone, select it and press the 'play' button to build and install.

Note that .critical alerts on iOS require the criticalAlert entitlement from Apple — without it, the system will fall back to .active behavior, but the code should still work for normal alarms.
