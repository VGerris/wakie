...
function MainApp() {
 ...
 // 3. Sync alarm firing state from context to modal visibility.
    // The native layer (AlarmService on Android, AVAudioPlayer on iOS) already
    // plays the alarm sound. The JS layer only controls the UI (modal visibility).
    // We do NOT play sound here to avoid double-sound.
    useEffect(() => {
    if (isAlarmFiring) {
      setIsAlarmModalVisible(true);
    } else {
